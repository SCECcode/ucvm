...
#include "ucvm_model_plugin.h"
     ...
     extern int cvmhvbn_setparam;
        ...
        extern int uwsfbcvm_init;
        extern int uwsfbcvm_query;
        extern int uwsfbcvm_finalize;
        extern int uwsfbcvm_version;
        extern int uwsfbcvm_config;
        extern int uwsfbcvm_setparam;
#endif
...
#endif
#ifdef _UCVM_ENABLE_UWSFBCVM
        if (strcmp(conf->label, UCVM_MODEL_UWSFBCVM) == 0) {
                pptr->model_init = &uwsfbcvm_init;
                pptr->model_query = &uwsfbcvm_query;
                pptr->model_finalize = &uwsfbcvm_finalize;
                pptr->model_version = &sfbvm_version;
                pptr->model_config = &uwsfbcvm_config;
                pptr->model_setparam = &uwsfbcvm_setparam;
                if ((*pptr->model_init)(conf->config, conf->label) != 0) {
                        fprintf(stderr, "Failed to initialize model, %s.\n", conf->label);
                        return UCVM_CODE_ERROR;
                }
...
fprintf(stderr,"XXX looking for sofile  %s\n", sofile);
