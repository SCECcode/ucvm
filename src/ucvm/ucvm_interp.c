...
/* ShakeOut2 interpolation method 
   so2+ but only apply to none-basin points */
...
/* XXXX NEED TO CHECK IF THE POINT IS IN BASIN OR NOT
       what about a very shallow basin */
    ...
    f = z - pow(z, 2.0);
    g = pow(z, 2.0) + 2*pow(z, 0.5) - 3*z;
    double so2_data_cmb_vs = (z + ucvm_interp_ely_b*f)*(data->crust.vs) + 
      (ucvm_interp_ely_a - ucvm_interp_ely_a*z + 
       ucvm_interp_ely_c*g)*data->gtl.vs;
    double so2_data_cmb_vp = (z + ucvm_interp_ely_b*f)*(data->crust.vp) + 
      (ucvm_interp_ely_a - ucvm_interp_ely_a*z + 
       ucvm_interp_ely_c*g)*ucvm_brocher_vp(data->gtl.vs);
    double so2_data_cmb_rho = ucvm_nafe_drake_rho(so2_data_cmb_vp);


    if(data->interp_crust.vs < so2_data_cmb_vs) {
      data->cmb.vs = data->interp_crust.vs;
      data->cmb.vp = data->interp_crust.vp;
      data->cmb.rho = data->interp_crust.rho;
      } else {
        data->cmb.vs = so2_data_cmb_vs;
        data->cmb.vp = so2_data_cmb_vp;
        data->cmb.rho = so2_data_cmb_rho;
    }
