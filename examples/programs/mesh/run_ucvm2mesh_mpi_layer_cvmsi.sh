...
salloc ${UCVM_SALLOC_ENV} -Q --nodes=2 --ntasks=4 --ntasks=4 --time=00:30:00 srun -Q -o ${TEST}_1.srun.out ./ucvm2mesh_mpi_layer -f la_habra_cvmsi.conf -l 1 -c 3
salloc ${UCVM_SALLOC_ENV} -Q --nodes=1 --ntasks=4 --time=00:30:00 srun -Q -o ${TEST}_2.srun.out ./ucvm2mesh_mpi_layer -f la_habra_cvmsi.conf -l 4 -c 3 
salloc ${UCVM_SALLOC_ENV} -Q --nodes=1 --ntasks=4 --time=00:30:00 srun -Q -o ${TEST}_3.srun.out ./ucvm2mesh_mpi_layer -f la_habra_cvmsi.conf -l 7 -c 4 

expect=$(mktemp) || exit 1
result=$(mktemp) || (trap 'rm -f "$expect"'; exit 1)

od -N 100 la_habra_cvmsi.media_vs |head -10 | sed 's/ //g' > $result 2>& 1
...
0000120015341041533015341041533015341041533017270041434
0000140017270041434
0000144
EOF_EXPECTED_RESULT
