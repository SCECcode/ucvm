...
TEST=basin_query_mpi_cencal_cvms

expect=$(mktemp) || exit 1
result=$(mktemp) || (trap 'rm -f "$expect"'; exit 1)

salloc -Q --ntasks=2 --time=00:10:00 srun -Q --ntasks=2 -o ${TEST}.srun.out ${BIN_DIR}/basin_query_mpi -b ${TEST}.simple -f ${CONF_DIR}/ucvm.conf -m cencal,cvms -i 20 -v 2500 -l 35.0,-122.5 -s 0.1 -x 16 -y 11
...
echo "Running examples_programs_basin basin_query_mpi_cencal_cvms"
